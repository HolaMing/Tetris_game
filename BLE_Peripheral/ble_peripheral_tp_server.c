...
static void ble_tp_ind_ccc_changed(const struct bt_gatt_attr *attr, u16_t value)
{
    ...
    printf("indicateeeeeeeeeeeeeeeeeeee\r\n");

    if (value == BT_GATT_CCC_INDICATE) {
        err = bl_tp_send_indicate(ble_tp_conn, get_attr(BT_CHAR_BLE_TP_IND_ATTR_VAL_INDEX), data, strlen(data));
...
static void ble_tp_notify_task(void *pvParameters)
{
    ...
    while (1) {
        ...
        err = bt_gatt_notify(ble_tp_conn, get_attr(BT_CHAR_BLE_TP_NOT_ATTR_VAL_INDEX), data, strlen(data));
        // data[0] = data[0] + 1;
...
static void ble_tp_notify_ccc_changed(const struct bt_gatt_attr *attr, u16_t value)
{
    ...
    printf("notifyyyyyyyyyyyyyyyyyyyyyyyyyyyy\r\n");
